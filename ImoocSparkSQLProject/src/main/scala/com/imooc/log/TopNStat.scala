...
object TopNStat {
  def main(args: Array[String]) {
    val spark = SparkSession.builder().appName("TopNStat").config("spark.sql.sources.partitionColumnTypeInference.enabled","false")
        .master("local[2]").getOrCreate()


    val accessDF = spark.read.format("parquet").load("/Users/zhanghongyu/Documents/LogAnalysis/clean")
...
//    accessDF.show(false)
  ...
  def videoAccessTopNStat(spark:SparkSession,accessDF:DataFrame,day:String)={
    ...
    val videoAccessTopNDF = accessDF.filter($"day" === day && $"cmsType" === "video").groupBy("day","cmsId").agg(count("cmsId").
